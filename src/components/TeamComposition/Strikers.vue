                ...
                <div id="slider" class="slider">
                    <div class="slider__wrap">
                        <swiper :options="swiperOption" @slideChange="changeSwiperIndex" ref="mySwiper">
                            <swiper-slide class="slider__item" v-for="(img, index, id) in players.striker"
               ...
               <div class="statistics-body">
                   <div class="statistics__header">
                       <h2 class="statistics__head">
                           {{players.striker[active].name}}
                       </h2>
                       <div class="statistics__tagline">
                            <span class="statistics__subtitle">
                                {{players.striker[active].report.club}}
                            </span>
                           <span class="statistics__subtitle">/</span>
                           <span class="statistics__subtitle">
                                {{players.striker[active].report.country}}
                            </span>
                       </div>
                   </div>
                   <div class="statistics-counter">
                       <div class="statistics-counter__box">
                           <div class="statistics-counter__number">
                               {{players.striker[active].description.playedMinutes}}
                           </div>
                           <p class="statistics-counter__info">Сыгранных минут</p>
                       </div>
                       <div class="statistics-counter__box">
                           <div class="statistics-counter__number">
                               {{players.striker[active].description.passedGoals}}
                           </div>
                           <p class="statistics-counter__info">Успешные передачи</p>
                       </div>
                       <div class="statistics-counter__box">
                           <div class="statistics-counter__number">
                               {{players.striker[active].description.shotsGoal}}
                           </div>
                           <p class="statistics-counter__info">Ударыпо воротам</p>
                       </div>
                       <div class="statistics-counter__box">
                           <div class="statistics-counter__number">
                               {{players.striker[active].description.goal}}
                           </div>
                           <p class="statistics-counter__info">Забитые голы</p>
                       </div>
                   </div>
                   <div class="summary">
                       <transition-group name="staggered-fade" tag="ul" class="summary-list">
                           <li class="summary-list__item"
                               v-for="(item, i) in players.striker[active].brief" :key="i">
                               {{item}}
                           </li>
                       </transition-group>
                   </div>
                   <div class="btn-wrap">
                       <div class="btn btn_main"
                            :class="{ 'btn_selected': players.striker[active].selected}"
                            @click="addPlayer('striker', players.striker[active])">
                           {{ players.striker[active].selected ? 'Выбран' : 'Добавить'}}
                       </div>
                   </div>
               </div>
            </div>
