                ...
                <div id="slider" class="slider">
                    <div class="slider__wrap">
                        <swiper :options="swiperOption" @slideChange="changeSwiperIndex" ref="mySwiper">
                            <swiper-slide class="slider__item" v-for="(img, index, id) in players.coach"
                ...
                <div class="statistics-body">
                    <div class="statistics__header">
                        <h2 class="statistics__head">
                            {{players.coach[active].name}}
                        </h2>
                        <div class="statistics__tagline">
                            <span class="statistics__subtitle">
                                {{players.coach[active].report.club}}
                            </span>
                            <span class="statistics__subtitle">/</span>
                            <span class="statistics__subtitle">
                                {{players.coach[active].report.country}}
                            </span>
                        </div>
                    </div>
                    <div class="summary">
                        <transition-group name="staggered-fade" tag="ul" class="summary-list">
                            <li class="summary-list__item" v-for="(item, i) in players.coach[active].brief" :key="i">
                                {{item}}
                            </li>
                        </transition-group>
                    </div>
                    <div class="btn-wrap">
                        <div class="btn btn_main" :class="{ 'btn_selected':  players.coach[active].selected}"
                             @click="addPlayer('coach', players.coach[active])">
                            {{ players.coach[active].selected ? 'Выбран' : 'Добавить'}}
                        </div>
                    </div>
                </div>
            </div>
